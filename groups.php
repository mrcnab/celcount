<?
	...
	$searchFrom	=	$_REQUEST['from'];
	$searchTo	=	$_REQUEST['to'];

    $q = "SELECT * FROM tbl_groups WHERE `group_init` >= '".$searchFrom."' AND `group_init` <= '".$searchTo."'";
	$q1 = "SELECT count( * ) as total FROM tbl_groups WHERE `group_init` >= '".$searchFrom."' AND `group_init` <= '".$searchTo."'";
...
<title>Search Group <?=strtoupper($searchFrom)?> To <?=strtoupper($searchTo)?></title>
...
function joinGroup(group_id) {
	...
	document.getElementById('msg').innerHTML = 'Successfully Added To Group';
}
...
function addToFavGroup(group_id) {
	...
	$.ajax({
		...
		document.getElementById('msg').innerHTML = 'Successfully Added This Group As Favourite';
}
</script>