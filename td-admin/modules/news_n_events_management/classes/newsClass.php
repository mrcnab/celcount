...
class news_and_events
{
	...
	function get_news_info( $news_id, $status = 0 )
	{
		...
		$q = "SELECT * FROM tbl_news_and_events WHERE ".$criteria." news_id = ".$news_id;
	...
	function get_active_news( $limit = "" )
	{
		...
		$q = "SELECT * FROM tbl_news_and_events WHERE status = 1 ORDER BY addeddate DESC".$limit;
	...
	{
		$limit = $limit != "" ? " LIMIT 0, ".$limit : "";
		$q = "SELECT * FROM tbl_news_and_events WHERE status = 0 ORDER BY addeddate DESC".$limit;
		$r = $this -> db -> getMultipleRecords( $q );
		if( $r )
			return $r;
		else
	...
	{
		$limit = $limit != "" ? " LIMIT 0, ".$limit : "";
		$q = "SELECT * FROM tbl_news_and_events ORDER BY addeddate DESC".$limit;
		$r = $this -> db -> getMultipleRecords( $q );
		if( $r )
			return $r;
		else
	...
	{
		$q = "SELECT news_title FROM tbl_news_and_events WHERE news_id = ".$news_id;
		$r = $this -> db -> getSingleRecord( $q );
		if( $r != "" )
			return $r['news_title'];
		else
	...
	{
		$q = "SELECT news_text FROM tbl_news_and_events WHERE news_id = ".$news_id;
		$r = $this -> db -> getSingleRecord( $q );
		if( $r != "" )
			return $r['news_text'];
		else
	...
	{
		$q = "SELECT news_status FROM tbl_news_and_events WHERE news_id = ".$news_id;
		$r = $this -> db -> getSingleRecord( $q );
		if( $r != "" )
	...
	function set_news_status( $news_id )
	{
		...
		$status = $status == 1 ? 0 : 1;
		$q = "UPDATE tbl_news_and_events SET news_status = ".$status." WHERE news_id = ".$news_id;
	...
	{
		$q = "INSERT INTO tbl_news_and_events(`news_title`, `news_text`, `news_status`, `addeddate`, `modifieddate`) VALUES('".$newsTitle."', '".$newsText."', '".$status."', '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."')";
	...
	{
		$q = "UPDATE tbl_news_and_events SET `news_title` = '".$newsTitle."', `news_text` = '".$newsText."', `news_status` = '".$status."', `modifieddate` = '".date('Y-m-d H:i:s')."' WHERE news_id = ".$news_id;
		$r = $this -> db -> updateRecord( $q );
		if( $r )
			return true;
		else
	...
	{
		$q = "DELETE FROM tbl_news_and_events WHERE news_id = ".$news_id;
