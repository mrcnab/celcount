...
class contents
{
	...
	function get_active_contents( $limit = "" )
	{
		...
		$q = "SELECT * FROM tbl_contents WHERE content_status = 1 ORDER BY addeddate DESC".$limit;
	...
	{
		$limit = $limit != "" ? " LIMIT 0, ".$limit : "";
		$q = "SELECT * FROM tbl_contents WHERE content_status = 0 ORDER BY addeddate DESC".$limit;
		$r = $this -> db -> getMultipleRecords( $q );
		if( $r != false )
			return $r;
		else
	...
	{
		$limit = $limit != "" ? " LIMIT 0, ".$limit : "";
		$q = "SELECT * FROM tbl_contents ORDER BY addeddate DESC".$limit;
		$r = $this -> db -> getMultipleRecords( $q );
		if( $r != false )
			return $r;
		else
	...
	{
		$q = "SELECT content_id FROM tbl_contents WHERE content_title = '".$content_title."'";
		$r = $this -> db -> getSingleRecord( $q );
		if( $r != false )
			return $r['content_id'];
		else
	...
	{
		$q = "SELECT content_title FROM tbl_contents WHERE content_id = ".$content_id;
		$r = $this -> db -> getSingleRecord( $q );
		if( $r != false )
	...
	{
		$q = "SELECT content_text FROM tbl_contents WHERE content_id = ".$content_id;
		$r = $this -> db -> getSingleRecord( $q );
		if( $r != false )
	...
	{
		$q = "SELECT content_status FROM tbl_contents WHERE content_id = ".$content_id;
		$r = $this -> db -> getSingleRecord( $q );
		if( $r != false )
	...
	function set_content_status( $content_id )
	{
		...
		$status = $status == 1 ? 0 : 1;
		$q = "UPDATE tbl_contents SET content_status = ".$status." WHERE content_id = ".$content_id;
	...
	{
		$q = "UPDATE tbl_contents SET `parent_id` = '".$parent_id."', `content_title` = '".$content_title."', `content_text` = '".$content_text."', `meta_title` = '".$meta_title."', `meta_description` = '".$meta_description."', `meta_keywords` = '".$meta_keywords."', `sef_link` = '".$sef_link."', `content_status` = '".$content_status."', `modifieddate` = '".date('Y-m-d H:i:s')."' WHERE content_id = ".$content_id;
		$r = $this -> db -> updateRecord( $q );
		if( $r != false )
			return true;
		else
	...
	function add_content( $parent_id, $content_title, $content_text, $meta_title, $meta_description, $meta_keywords, $sef_link, $content_status )
	{
		...
		{
			$q = "INSERT INTO tbl_contents(`parent_id`, `content_title`, `content_text`, `meta_title`, `meta_description`, `meta_keywords`, `sef_link`, `content_status`, `addeddate`, `modifieddate`)
	...
	{
		$q = "SELECT content_id FROM tbl_contents WHERE parent_id = ".$content_id;
		$r = $this -> db -> getMultipleRecords( $q );
		if( $r != false )
			return true;
		else
	...
	function delete_content( $content_id )
	{
		...
		{	
			$q = "DELETE FROM tbl_contents WHERE content_id = ".$content_id;
	...
	{
		$q = "SELECT * FROM tbl_contents WHERE parent_id = 0";
		$r = $this -> db -> getMultipleRecords( $q );
